...
2. re-pastes the photo's own notch (camera dot included) using its dark
   pixels only, so no old-wallpaper fringe comes along;
...
NX0, NX1, NY0, NY1 = 916, 1122, 36, 92
bbox = np.zeros((H, W), dtype=bool)
bbox[NY0:NY1, NX0:NX1] = True
notch = (g < 55) & bbox
out.paste(base, (0, 0), Image.fromarray((notch * 255).astype("uint8"), "L").filter(ImageFilter.GaussianBlur(0.6)))
